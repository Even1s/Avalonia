...
public partial class WaiterReportWindow : Window
{
    ...
    private string MakeReport(Shift shift)
    {
        ...
        return $"""
               ...
               Общая выручка: {orders.Sum(x => x.Payment)}
                 наличными - {orders.Where(x => x.PaymentMethod == PaymentMethods.CASH).Sum(x => x.Payment)}
                 безналичным способом - {orders.Where(x => x.PaymentMethod == PaymentMethods.NON_CASH).Sum(x => x.Payment)}
               Общее кол-во клиентов: {orders.Sum(x => x.ClientsNumber)}
               """;
    }
}